...
5. Run `npm run video:capture -- --plan video/plans/<slug>.json`,
   `npm run video:assets -- --plan video/plans/<slug>.json --required`, and
...
7. Review every `verification/final-*.jpg` image in each output for burned
